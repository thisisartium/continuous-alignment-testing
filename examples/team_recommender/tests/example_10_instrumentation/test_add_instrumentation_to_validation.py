...
class Validator:
    def __init__(self, validation: Callable):
        """
        Initializes the Validator with a validation function.

        :param validation: A callable that takes a value
        """
        self.validation = validation

    def validate(self, *args, **kwargs):
        """
        Validates the given arguments using the provided validation function.

        :param args: Positional arguments to validate.
        :param kwargs: Keyword arguments to validate.
        :return: True if the arguments are valid, False otherwise.
        """
        return self.validation(*args, **kwargs)
...
def run_allocation_test(reporter: Reporter, skills_data, response: str) -> bool:
    ...
    json_object = {}
    schema_validator = Validator(response_matches_json_schema)
    try:
        json_object = json.loads(response)
        has_valid_json_schema = schema_validator.validate(json_object, schema)
